...
df['Region'] = df['Region'].replace('Total, New Zealand by territorial authority/SA2', 'Total, New Zealand')
...
df['Generation'] = df.apply(lambda row: determine_generation(row['Year'], row['Age']), axis=1)


# Add a column that marks if the Age group is 65 years and over
old_age_groups=['65-69 Years', '70-74 Years', '75-79 Years',
